...
def var_process(gmn_input, annotsv_input, output_filename):
    ...
    output_header_list = ["chrom", "start", "stop", "sv_type", "overlap_CDS_percent", "de_novo_anno", "patho_gain_list[phen|hpo|source|coord]", "patho_loss_list[phen|hpo|source|coord]", "benign_gain_list[source|coord]", "benign_loss_list[source|coord]", "cohort_freq", "genes", "individuals"]
    ...
    for a in range(1, len(annotsv_list)-1):
        ...
        for b in range(1, len(gmn_list)-2):
            ...
            # retain if SVID matches AND there is de novo annotation AND annotation mode is full
            de_novo_field = gene_list[6]
            if(var_svid in gene_svid and de_novo_field != "" and anno_mode == "full"):
                # assign Geminesque data
                de_novo_anno = "family" + de_novo_field.split("family")[1]
                # write out whole list
                output_list = [str(sv_chrom), str(sv_start), str(sv_stop), sv_type, str(overlap_CDS_percent), de_novo_anno, p_gain_string, p_loss_string, b_gain_string, b_loss_string, cohort_freq, gene_id, indvs_string]
                ...
                if(sv_chrom == 9 and sv_start == 134064728 and sv_stop == 139351737):
                    ...
                    print("anno_ bool: " + str(anno_mode == "full"))
...
def var_prioritize(input_file, output_file, disp_file, ndd_file, strictness):
    """
    var_prioritize()
...
# COMMANDS
phenotype = "ASD_only"
filepath = "/lab01/Projects/Rohan_Projects/CNV_Project/2022/Total_Pathway/Data/"
var_process(filepath + phenotype + "/GMN_out_" + phenotype + "_final.tsv", filepath + "CNV2_anno_corrected.tsv","../Data/" + phenotype + "/GMNP_intermediate1.tsv")
var_sort(filepath + phenotype + "/GMNP_intermediate1.tsv", filepath + phenotype + "/GMNP_intermediate2.tsv")
var_prioritize(filepath + phenotype + "/GMNP_intermediate2.tsv", filepath + phenotype + "/GMNP_intermediate3.tsv", filepath + "dispensable_genes_and_muc.txt", filepath + "NDD_genes.tsv", "lenient")
